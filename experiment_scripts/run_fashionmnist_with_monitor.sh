...
JOIN_RATIOS=(0.1 0.25 0.5 0.75 1.0)
REPETITIONS=5
...
for JR in "${JOIN_RATIOS[@]}"; do
    for REP in $(seq 1 "$REPETITIONS"); do
        ...
        # 0.1 -> 01 ; 0.25 -> 025 ; 0.5 -> 05 ; 0.75 -> 075 ; 1.0 -> 10
        JR_TAG=$(echo "$JR" | sed 's/\.//g')

        EXP_TAG="${ALGO}_GPU_JR${JR_TAG}_rep${REP}"
        EXP_DIR="$OUTPUT_BASE/jr${JR_TAG}/rep${REP}"
        ...
        SAVE_NAME="FashionMNIST_${ALGO}_gpu_jr${JR_TAG}_rep${REP}"
